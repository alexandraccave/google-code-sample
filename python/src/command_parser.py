...
class CommandParser:
    ...
    def execute_command(self, command: Sequence[str]):
        ...
        if command[0].upper() == "NUMBER_OF_VIDEOS":
            ...
            self.player.continue_video()

        elif command[0].upper() == "SHOW_PLAYING":
            self.player.show_playing()
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "playlist name.")
            self.player.create_playlist(command[1])
            ...
            self.player.add_to_playlist(command[1], command[2])
            ...
            self.player.remove_from_playlist(command[1], command[2])
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "playlist name.")
            self.player.clear_playlist(command[1])
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "playlist name.")
            self.player.delete_playlist(command[1])
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "playlist name.")
            self.player.show_playlist(command[1])

        elif command[0].upper() == "SHOW_ALL_PLAYLISTS":
            self.player.show_all_playlists()
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "search term.")
            self.player.search_videos(command[1])
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "video tag.")
            self.player.search_videos_tag(command[1])

        elif command[0].upper() == "FLAG_VIDEO":
            if len(command) == 3:
                self.player.flag_video(command[1], command[2])
            elif len(command) == 2:
                self.player.flag_video(command[1])
            else:
                raise CommandException(
            ...
            if len(command) != 2:
                raise CommandException(
                    ...
                    "video_id.")
            self.player.allow_video(command[1])
    ...
    def get_help(self):
        ...
        Available commands:
            ...
            SHOW_PLAYING - Displays the name, url and paused status of the video that is currently playing (or paused).
            ...
            SEARCH_VIDEOS <search_term> - Display all the videos whose names contain the search_term.
